...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *bgSprite = [SKSpriteNode spriteNodeWithImageNamed:@"office_v2"];
...
-(void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    if(contact.bodyA.categoryBitMask == GERM_CATEGORY && (contact.bodyB.categoryBitMask & test) > 0) {
        ...
        [self performSelector:@selector(winCheck:) withObject:contact.bodyA.node afterDelay:2.0];
    }
        ...
        [self performSelector:@selector(winCheck:) withObject:contact.bodyB.node afterDelay:2.0];
...
//        NSLog(@"node info: %@", node);
    ...
    if (!gameOn) {
        UIAlertView *noGameBox = [[UIAlertView alloc] initWithTitle:@"このステージは菌がいません"
                                                            message:@"だから、掃除できません。"
                                                           delegate:self
                                                  cancelButtonTitle:@"はい、分かりました"
...
//    NSLog(@"spriteNames: %@", sprite.name);
   ...
   if ([sprite.name isEqualToString:@"ByoBall1"] && !transitioned) {
        finishedGame = YES;
        wonGame = NO;
    ...
    if (!transitioned) {
        [self performSelector:@selector(winCheck:) withObject:sprite afterDelay:3.0];
    }*/
